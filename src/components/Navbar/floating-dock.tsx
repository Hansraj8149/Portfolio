    ...
    <div className={cn("relative block md:hidden", className)}>
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId="nav"
            className="absolute top-full -inset-x-24 flex gap-2"
          >
            {items.map((item, idx) => (
              <motion.div
                key={item.title}
              ...
              >
                <Link
                  href={item.href}
                  key={item.title}
                  className="h-10 w-10 rounded-full bg-background-light flex items-center justify-center"
                >
                  <div className="h-4 w-4 text-text-secondary">{item.icon}</div>
                </Link>
              </motion.div>
            ))}
          </motion.div>
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="px-2 py-0.5 whitespace-pre rounded-md border bg-primary border-border-light text-background absolute left-1/2 -translate-x-1/2 -bottom-8 w-fit text-xs"
            >
              {title}
            </motion.div>
          )}
